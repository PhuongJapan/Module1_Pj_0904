...
function createClass() {
  ...
  // let arrClass = localStorage.getItem("arrClass") ? JSON.parse(localStorage.getItem("arrClass")) : [];
  ...
  // let newClass = { classId, className, teacher, studentNumber, descriptions, status};
  ...
  let id = getCourseIdOfClass(arrCourse,courseNameId);
  if(id>=0){
    //. push dư liệu thêm mới vào arrCourse
    arrCourse[id].arrClass.push(newClass);
  }
...
function getDataForm() {
  ...
  let courseClass={classId,className, teacher,studentNumber,descriptions,status, arrStudent:[]
  }
  return courseClass;
}
...
//   let filterData = searchArrCourse.filter(class=>class.courseName.toLowerCase().includes(searchCourseName.toLowerCase()));
